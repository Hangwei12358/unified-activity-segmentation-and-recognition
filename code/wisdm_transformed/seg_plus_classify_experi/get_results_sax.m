...
test_txt_name = 'test1_svm_g0.1_c10.txt';
n_runs = 6;
numSaxs = (3:1:10)';
name_of_methods = {'Binseg'; 'BottomUp'; 'Dynp'; 'KCpA'; 'Pelt'; 'Window'}; 
num_methods = size(name_of_methods, 1);

results_sax = cell(1,1);
results_ind = 1;
for i = 1: num_methods
    for j = 1:n_runs
        now_folder_name = strcat('sax_', name_of_methods{i, 1}, '_', num2str(j));
        % get results from 4 subfolders
        cd(now_folder_name);
        for numSax = 1:size(numSaxs, 1)
            nowSax = numSaxs(numSax, 1);
            now_sub_folder_name = strcat('sax_', num2str(nowSax), '_experi');
            [miF_event, w_maF_event, miF_frame, w_maF_frame] = f_get_results_incNullClass_svm(now_sub_folder_name, test_txt_name);
            results_sax{results_ind, 1} = now_folder_name;          
            results_sax{results_ind, 2} = now_sub_folder_name;
            results_sax{results_ind, 3} = miF_event;
            results_sax{results_ind, 4} = w_maF_event;
            results_sax{results_ind, 5} = miF_frame;
            results_sax{results_ind, 6} = w_maF_frame;   
            results_sax{results_ind, 7} = strcat('sax_', num2str(nowSax), '_', name_of_methods{i, 1});   
            results_ind = results_ind + 1;
        end
        cd ..
    end
end 
save('results_sax.mat','results_sax');
% get the mean and std for each method
[results_table] = f_get_results_table_from_subfolders(results_sax, n_runs, numSaxs);
save('results_sax.mat', 'results_table', '-append');


function [results_table] = f_get_results_table_from_subfolders(results_sax, n_runs, numSaxs)
% learned from RESULTS_ANALYZE_SEMI_large.m
results_all_mat = cell2mat(results_sax(:, 3:6));
name_of_all_methods = results_sax(:, 7);
...
[confuMatrix, mi_frame, ma_frame] = micro_macro_PR_WISDM(calcu_label, label_t, proportion_each_label);
fclose(fID);
...
true_no0 = true_tmp(find(true_tmp(:,1)~= 0));
...
[confuMatrix_event, mi_event, ma_event] = micro_macro_PR_WISDM(predicted_tmp, true_tmp, proportion_each_label_event);
