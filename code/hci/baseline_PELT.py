...
# number_ind =  45000 # 99568 # full: 99568
# use full data
...
def runPartData(now_chunk_ind):
    ...
    algo = rpt.Pelt(model = model, custom_cost = c, jump = 1).fit(now_series_data)
...
if number_ind > n_parallel_chunk: 
    ...
    algo = rpt.Pelt(model = model, custom_cost = c, jump = 1).fit(series_data)
    # different parameter
    my_bkps = algo.predict(pen= 4)
    p, r = precision_recall(my_bkps, bkps_true)
    results_table.loc[now_ind] = ['Pelt', c.sum_of_costs(my_bkps),hausdorff(my_bkps, bkps_true), randindex(my_bkps, bkps_true), p, r]
    now_ind +=1
