...
results_dict = OrderedDict([('Method', 'optimal_measure'), ('cost', 0), ('Hausdorff',0), ('Rand', 1), ('P', 1), ('R', 1)])
...
n_parallel_chunk = 4000

series_data = series_data_full[1:(number_ind+1), 1:10]
...
def runPartDataDP(now_chunk_ind):
    ...
    algo = rpt.Dynp(model = model, custom_cost = c, jump = 1).fit(now_series_data)
...
if number_ind > n_parallel_chunk: 
    ...
    number_chunks = int((number_ind-number_ind % n_parallel_chunk)/n_parallel_chunk + 1)
    Parallel(n_jobs=-1)(delayed(runPartDataDP)(now_chunk_ind = i) for i in range(number_chunks))
    combine_txts()
else:
    algo = rpt.Dynp(model = model, custom_cost = c, jump = 1).fit(series_data)
    my_bkps = algo.predict(n_bkps=n_bkps)
    p, r = precision_recall(my_bkps, bkps_true)
    results_table.loc[now_ind] = ['Dynp', c.sum_of_costs(my_bkps), hausdorff(my_bkps, bkps_true), randindex(my_bkps, bkps_true), p, r]
    now_ind +=1
    np.savetxt(results_folder + now_method + '_'+ str(number_ind)+'.txt', my_bkps, fmt='%d')
    np.savetxt(results_folder + now_method + '_'+ str(number_ind)+'.txt', my_bkps, fmt='%d')
    results_table.to_csv(results_folder + results_folder + 'results_table_'+ now_method + '_'+ str(number_ind) + '.csv', sep = '\t')
...
# plt.show()
'''
print("# true bkps: " + str(n_bkps))
print("ground truth: " + str(bkps_true))
print("predicted: " + str(my_bkps))
print("sum of costs: " + str(c.sum_of_costs(my_bkps)))
print("Hausdorff metric: " + str(hausdorff(my_bkps, bkps_true)))
print("Rand index: " + str(randindex(my_bkps, bkps_true)))
p, r = precision_recall(my_bkps, bkps_true)
print("precision: " + str(p))
print("recall: " + str(r))
np.savetxt(str(model) +'_'+ str(number_ind)+'.txt', my_bkps, fmt='%d')
'''

# rpt.show.display(series_data, bkps_true, my_bkps, figsize=(10, 6))

'''
# single loop, easier for debugging, by Hangwei
for now_chunk_ind in range(number_chunks):
    print(now_chunk_ind)
    start_ind = now_chunk_ind * n_parallel_chunk + 1
    end_ind = min((now_chunk_ind + 1) * n_parallel_chunk, number_ind)
    now_series_data = series_data[start_ind:end_ind, :]
    now_bkps_true = bkps_true_full[ (start_ind < bkps_true_full) & (bkps_true_full < end_ind)]
    now_bkps_true = np.append(now_bkps_true, (end_ind-1))
    now_n_bkps = len(now_bkps_true) - 1
    algo = rpt.Dynp(model = model, custom_cost = c, jump = 1).fit(now_series_data)
    ...
    print('my_bkps:',my_bkps, 'now_bkps_true', now_bkps_true)
    p, r = precision_recall(my_bkps, now_bkps_true)
    results_table.loc[now_ind] = ['Dynp', c.sum_of_costs(my_bkps), hausdorff(my_bkps, now_bkps_true), randindex(my_bkps, now_bkps_true), p, r]
    now_ind +=1
    np.savetxt(now_method + '_'+ str(number_ind)+'_'+str(now_chunk_ind) +'.txt', my_bkps, fmt='%d')
    np.savetxt(now_method + '_'+ str(number_ind)+'+'+str(now_chunk_ind) +'.txt', my_bkps, fmt='%d')
    results_table.to_csv('results_table_'+ now_method + '_'+ str(number_ind) + '.csv', sep = '\t')
'''
