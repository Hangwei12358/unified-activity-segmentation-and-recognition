...
## RBF sliding window method
## rbf sliding window better than L2 sliding window better than L1
n_width = 100
model = "rbf"
now_method = "Window"
c = rpt.costs.CostRbf()  
...
def combine_txts():
    ...
    for now_file_ind in range(number_chunks):
        ...
        if now_file_ind < number_chunks-1: # need to remove the last ind
            ...
            print(now_data)
    print(results_list)
    np.savetxt(results_folder + now_method + '_'+ str(number_ind)+'_'+ str(n_width)+'.txt', results_list, fmt='%d')
    np.savetxt(results_folder + now_method + '_'+ str(number_ind)+'_'+ str(n_width)+'.txt', results_list, fmt='%d')
...
def runPartData(now_chunk_ind):
    ...
    algo = rpt.Window(model = model, custom_cost = c, width = n_width, jump = 1).fit(now_series_data)
...
if number_ind > n_parallel_chunk: 
    ...
    # Parallel(n_jobs=-1)(delayed(runPartData)(now_chunk_ind = i) for i in range(number_chunks-1, number_chunks))
    combine_txts()
else:
    algo = rpt.Window(model = model, custom_cost = c, width = n_width, jump = 1).fit(series_data)  # jump = 5/1 dont have much difference in result
    my_bkps = algo.predict(n_bkps=n_bkps)
    # p, r = precision_recall(my_bkps, bkps_true)
    # results_table.loc[now_ind] = ['Window', c.sum_of_costs(my_bkps), hausdorff(my_bkps, bkps_true), randindex(my_bkps, bkps_true), p, r]
    # now_ind +=1
    np.savetxt(results_folder + now_method + '_'+ str(number_ind)+'_'+ str(n_width)+'.txt', my_bkps, fmt='%d')
    np.savetxt(results_folder + now_method + '_'+ str(number_ind)+'_'+ str(n_width)+'.txt', my_bkps, fmt='%d')
    # results_table.to_csv(results_folder + 'results_table_'+ now_method + '_'+ str(number_ind)+'_'+ str(n_width)+ '.csv', sep = '\t')
