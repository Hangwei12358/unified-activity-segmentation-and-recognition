...
if number_ind > n_parallel_chunk: 
    ...
    results_table.loc[now_ind] = ['KCpA', c.sum_of_costs(my_bkps), hausdorff(my_bkps, bkps_true), randindex(my_bkps, bkps_true), p, r]
    now_ind +=1
    np.savetxt(results_folder + now_method+ '_'+cost_func + '_'+ str(number_ind)+'_'+ str(n_width)+'.txt', my_bkps, fmt='%d')
    np.savetxt(results_folder + now_method+ '_'+cost_func + '_'+ str(number_ind)+'_'+ str(n_width)+'.txt', my_bkps, fmt='%d')
    results_table.to_csv(results_folder + 'results_table_'+ now_method + '_'+ str(number_ind) + '.csv', sep = '\t')
