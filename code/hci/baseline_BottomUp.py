...
def runPartData(now_chunk_ind):
    ...
    algo = rpt.BottomUp(model = model, custom_cost = c, jump = 1).fit(now_series_data)
...
if number_ind > n_parallel_chunk: 
    ...
    algo = rpt.BottomUp(model = model, custom_cost = c, jump = 5).fit(series_data)
    my_bkps = algo.predict(n_bkps=n_bkps)
    p, r = precision_recall(my_bkps, bkps_true)
    results_table.loc[now_ind] = ['BottomUp', c.sum_of_costs(my_bkps), hausdorff(my_bkps, bkps_true), randindex(my_bkps, bkps_true), p, r]
    now_ind +=1
