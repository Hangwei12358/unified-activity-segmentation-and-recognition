% this script is to generate data for SAX feature extraction and SVM classification under various segmentation
% results.
% learn from experi_ECDF_classify_with_various_seg.m
...
for tmpFolderIndn = 1: numRuns    
    ...
    tmpFolderPath = strcat('sax_', now_method, '_', tmpFolderInd);
...
for nowSaxInd = 1:length(numSax)
nowSax = numSax(nowSaxInd, 1);  
saxFolderPath = strcat('sax_', num2str(nowSax), '_experi');
% if(~exist(saxFolderPath, 'dir'))
    mkdir(saxFolderPath);
    fileattrib(saxFolderPath, '+w');
% end
cd(saxFolderPath);
...
for tmpClass = 1: length(allLabels) % class
    ...
    for i = 1:size(trainDataPool, 1)
        if(ismember(i, trainSubject))            
            tmpData = trainDataPool{i, 1}; % data         
            [trainCell{trainInd, 1}, ~] = timeseries2symbol(tmpData, size(tmpData, 1), size(tmpData,2), nowSax);
            trainCell{trainInd, 2} = trainDataPool{i, 2}; % class label
            trainInd = trainInd + 1;
        else
            tmpData = testDataPool{i, 1};
            [testCell{testInd, 1}, ~] = timeseries2symbol(tmpData, size(tmpData, 1), size(tmpData,2), nowSax);
